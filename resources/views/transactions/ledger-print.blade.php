...
<head>
    ...
    <style>
        * {
        ...
        /* Customer Section - Same as Invoice Bill Section */
        .customer-section {
            display: grid;
        ...
        .customer-details, .ledger-summary {
            border: 1px solid #e5e7eb;
            padding: 20px;
            border-radius: 8px;
            background: #f9fafb;
        ...
        .customer-details p, .ledger-summary p {
            margin: 6px 0;
            font-size: 13px;
            color: #374151;
            line-height: 1.4;
    ...
    <div class="ledger-container">
        ...
        <div class="customer-section">
            ...
            <div class="ledger-summary">
                ...
                <p>Opening Balance: <span class="{{ $customer->opening_balance >= 0 ? 'balance-positive' : 'balance-negative' }}">{{ number_format($customer->opening_balance, 2) }}</span></p>
                <p>Current Balance: <span class="{{ $customer->outstanding_balance >= 0 ? 'balance-positive' : 'balance-negative' }}">{{ number_format($customer->outstanding_balance, 2) }}</span></p>
        ...
        <div class="table-container">
            <table class="ledger-table">
                ...
                <tbody>
                    @php $runningBalance = $customer->opening_balance; @endphp
                    ...
                    <tr class="opening-balance-row">
                        ...
                        <td class="amount-cell">-</td>
                        <td class="amount-cell">-</td>
                        <td class="amount-cell {{ $runningBalance >= 0 ? 'amount-positive' : 'amount-negative' }}">{{ number_format($runningBalance, 2) }}</td>
                    </tr>
                    ...
                    @foreach($transactions as $transaction)
                        ...
                        <tr class="{{ $hasDiscount ? 'discount-row' : '' }}">
                            ...
                            <td>
                                <div class="transaction-description">{{ $transaction->purpose }}</div>
                                @if($hasDiscount && $transaction->discount_reason)
                                    <div class="transaction-note">Discount: {{ $transaction->discount_reason }}</div>
                                @endif
                                @if($transaction->reference)
                                    <div class="transaction-note">Ref: {{ $transaction->reference }}</div>
                                @endif
                            </td>
                            ...
                            @if($transaction->type == 'debit')
                                <td class="amount-cell amount-negative">{{ number_format($transaction->amount, 2) }}</td>
                                <td class="amount-cell {{ $hasDiscount ? 'amount-positive' : '' }}">
                                    {{ $hasDiscount ? number_format($transaction->discount_amount, 2) : '-' }}
                                </td>
                                <td class="amount-cell">-</td>
                            @else
                                <td class="amount-cell">-</td>
                                <td class="amount-cell">-</td>
                                <td class="amount-cell amount-positive">{{ number_format($transaction->amount, 2) }}</td>
                            @endif
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="amount-cell"><strong>{{ number_format($totalPayments, 2) }}</strong></td>
                        <td class="amount-cell"><strong>{{ number_format($totalDiscounts, 2) }}</strong></td>
                        <td class="amount-cell"><strong>{{ number_format($totalCredits, 2) }}</strong></td>
                        <td class="amount-cell"><strong>{{ number_format($customer->outstanding_balance, 2) }}</strong></td>
                    </tr>
                </tfoot>
            </table>
        </div>
        ...
        <div class="summary-section">
            <div class="summary-info">
                ...
                <p>• Total Discounts: ৳{{ number_format($totalDiscounts, 2) }}</p>
                <p>• Total Credits: ৳{{ number_format($totalCredits, 2) }}</p>
                <p>• Net Transaction Amount: ৳{{ number_format($totalEffectiveDebits - $totalCredits, 2) }}</p>
                @if($totalDiscounts > 0)
                    <p><strong>Note:</strong> {{ $transactions->where('discount_amount', '>', 0)->count() }} transactions included discounts.</p>
                @endif
            </div>
            ...
            <div class="balance-totals">
                <table>
                    ...
                    <tr>
                        ...
                        <td class="amount-positive">{{ number_format($totalDiscounts, 2) }}</td>
                    </tr>
                    <tr>
                        <th>Total Credits:</th>
                        <td class="amount-positive">{{ number_format($totalCredits, 2) }}</td>
                    </tr>
                    <tr class="total-row">
                        <th>Current Balance:</th>
                        <td class="{{ $customer->outstanding_balance >= 0 ? 'amount-positive' : 'amount-negative' }}">{{ number_format($customer->outstanding_balance, 2) }}</td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="disclaimer">
            This ledger statement shows all transactions for the specified customer account. 
